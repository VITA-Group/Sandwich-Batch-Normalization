...
lib_dir = (Path(__file__).parent / 'lib').resolve() # noqa
if str(lib_dir) not in sys.path: sys.path.insert(0, str(lib_dir))   # noqa
from config_utils import load_config, dict2config
from datasets import get_datasets, get_nas_search_loaders
from procedures import prepare_seed, prepare_logger, save_checkpoint, copy_checkpoint, get_optim_scheduler
from utils import get_model_infos, obtain_accuracy, random_alpha, get_per_egde_value_dict
...
def search_func(xloader, network, criterion, scheduler, w_optimizer, a_optimizer, epoch_str, print_freq, logger,
                gradient_clip, perturb_alpha, epsilon_alpha):
    ...
    for step, (base_inputs, base_targets, arch_inputs, arch_targets) in enumerate(xloader):
        ...
        if perturb_alpha:
            network.module.softmax_arch_parameters()
            perturb_alpha(network, epsilon_alpha)
        ...
        if perturb_alpha:
            network.module.restore_arch_parameters()
...
def main(xargs):
    ...
    if os.path.isdir(xargs.save_dir):
        if click.confirm('\nSave directory already exists in {}. Erase?'.format(xargs.save_dir, default=False)):
    ...
    if xargs.model_config is None:
        model_config = dict2config({'name': xargs.model, 'C': xargs.channel, 'N': xargs.num_cells,
    ...
    for epoch in range(start_epoch, total_epoch):
        ...
        search_w_loss, search_w_top1, search_w_top5, search_a_loss, search_a_top1, search_a_top5 = search_func(
            search_loader, network, criterion, w_scheduler,
            w_optimizer, a_optimizer, epoch_str, xargs.print_freq,
            logger, xargs.gradient_clip, perturb_alpha, epsilon_alpha)
        ...
        if xargs.snapshoot > 0 and epoch % xargs.snapshoot == 0:
            save_checkpoint({
                ...
                'args': deepcopy(args),
                'search_model': search_model.state_dict(),
            }, os.path.join(str(logger.model_dir), f"checkpoint_epoch{epoch}.pth"), logger)
        ...
        if api is not None:
            ...
            res_metrics = info.get_metrics(f'{xargs.dataset}', 'ori-test')  # This is a dict with metric names as keys
            # cost_metrics = info.get_comput_costs('cifar10')
            writer.add_scalar(f'{xargs.dataset}_ground_acc_ori-test', res_metrics['accuracy'], epoch)
            writer.add_scalar(f'{xargs.dataset}_search_acc', valid_a_top1, epoch)
            if xargs.dataset.lower() != 'cifar10':
                writer.add_scalar(f'{xargs.dataset}_ground_acc_x-test',
                                  info.get_metrics(f'{xargs.dataset}', 'x-test')['accuracy'],
                                  epoch)
            if find_best:
                valid_accuracies['best_gt'] = res_metrics['accuracy']
            writer.add_scalar(f"{xargs.dataset}_cur_best_gt_acc_ori-test", valid_accuracies['best_gt'], epoch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str,
                        choices=['DARTS-V1', 'DARTS-V1-ccbn', 'DARTS-V1-sabn'],
                        required=True, help='The path of the configuration.')
    # architecture leraning rate
    parser.add_argument('--arch_learning_rate', type=float, default=3e-4, help='learning rate for arch encoding')
    parser.add_argument('--weight_learning_rate', type=float, default=0.025, help='learning rate for model weight')
    ...
    parser.add_argument('--affine', type=int, choices=[0, 1],
                        help='Whether use learnable param or not in the BN layer.')
    parser.add_argument('--perturb', action='store_true', help='perturb or not')
    parser.add_argument('--epsilon_alpha', type=float, default=0.3, help='max epsilon for alpha')
    parser.add_argument('--snapshoot', type=int, default=0, help='epoch-wise checkpoint snapshoot')
    args = parser.parse_args()

    if args.model in ['DARTS-V1-sabn']:
        assert args.affine
