...
if str(lib_dir) not in sys.path:
    sys.path.insert(0, str(lib_dir))  # noqa
...
def main(xargs):
    ...
    for epoch in range(start_epoch, total_epoch):
        ...
        if xargs.snapshoot > 0 and epoch % xargs.snapshoot == 0:
            save_checkpoint(
                ...
                os.path.join(str(logger.model_dir), f"checkpoint_epoch{epoch}.pth"),
                logger,
            )
        ...
        if api is not None:
            ...
            info = api.query_meta_info_by_index(
                index
            )  # This is an instance of `ArchResults`
            res_metrics = info.get_metrics(
                f"{xargs.dataset}", "ori-test"
            )  # This is a dict with metric names as keys
            # cost_metrics = info.get_comput_costs('cifar10')
            writer.add_scalar(
                f"{xargs.dataset}_ground_acc_ori-test", res_metrics["accuracy"], epoch
            )
            ...
            writer.add_scalar(
                f"{xargs.dataset}_cur_best_gt_acc_ori-test",
                valid_accuracies["best_gt"],
                epoch,
            )
    ...
    if api is not None:
        logger.log(
            "{:}".format(api.query_by_arch(genotypes[xargs.early_stop_epoch - 1]))
        )
    logger.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.025,
        help="learning rate for model weight",
    )
    parser.add_argument(
        "--arch_weight_decay",
        type=float,
    ...
    parser.add_argument(
        "--epsilon_alpha", type=float, default=0.3, help="max epsilon for alpha"
    )
    parser.add_argument(
        "--snapshoot", type=int, default=0, help="epoch-wise checkpoint snapshoot"
    )
