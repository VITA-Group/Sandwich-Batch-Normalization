...
def get_optim_scheduler(parameters, config, args_epoch=None, args_weight_lr=None):
    ...
    assert hasattr(config, 'optim') and hasattr(config, 'scheduler') and hasattr(config,
        ...
        config)
    if config.optim == 'SGD':
        optim = torch.optim.SGD(parameters, weight_lr, momentum=config.momentum, weight_decay=config.decay,
                                nesterov=config.nesterov)
    elif config.optim == 'RMSprop':
        optim = torch.optim.RMSprop(parameters, weight_lr, momentum=config.momentum, weight_decay=config.decay)
    else:
    ...
    if config.scheduler == 'cos':
        ...
        scheduler = MultiStepLR(optim, config.warmup, epochs, config.milestones, config.gammas)
    elif config.scheduler == 'exponential':
        scheduler = ExponentialLR(optim, config.warmup, epochs, config.gamma)
    elif config.scheduler == 'linear':
        scheduler = LinearLR(optim, config.warmup, epochs, weight_lr, config.LR_min)
    else:
