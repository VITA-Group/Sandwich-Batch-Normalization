...
def main(args):
    ...
    train_loader = torch.utils.data.DataLoader(
        train_datasets, batch_size=args.batch_size, sampler=train_sampler
    )

    val_loader = torch.utils.data.DataLoader(
        train_datasets, batch_size=args.batch_size, sampler=valid_sampler
    )
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        # clean branch
        test_tacc_clean, test_tloss_clean = validate(
        ...
        test_atacc_adv, test_atloss_adv = validate_adv(
            args, test_loader, model, criterion, 1
        )
