...
def train_func(args, network: nn.Module, optimizer, style_iter, content_iter, device, num_iter, save_dir, writer=None):
    network.train()
    ...
    if writer:
        ...
        if (num_iter + 1) % args.save_model_interval == 0 or (num_iter + 1) == args.max_iter or num_iter == 0:
            if args.saadain:
                states = {
            ...
            torch.save(states, save_dir /
                       'ckpt_iter_{:d}.pth.tar'.format(num_iter + 1))
            with torch.no_grad():
                if writer:
...
@torch.no_grad()
def val_func(args, network: nn.Module, style_iter, content_iter, device, num_iter, writer=None):
    print("=> validating...")
    network.eval()
    content_losses = []
    style_losses = []
