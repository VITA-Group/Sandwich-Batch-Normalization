...
class LandmarkDataset(data.Dataset):
    ...
    def __init__(self, transform, sigma, downsample, heatmap_type, shape, use_gray, mean_file, data_indicator,
                 cache_images=None):
    ...
    def __repr__(self):
        return (
            '{name}(point-num={NUM_PTS}, shape={shape}, sigma={sigma}, heatmap_type={heatmap_type}, length={length}, cutout={cutout}, dataset={dataset_name}, mean={mean_file})'.format(
    ...
    def reset(self, num_pts=-1, boxid='default', only_pts=False):
        ...
        if self.mean_data is None:
            ...
            assert (self.mean_face >= -1).all() and (self.mean_face <= 1).all(), 'mean-{:}-face : {:}'.format(boxid,
                                                                                                              self.mean_face)
    ...
    def load_list(self, file_lists, num_pts, boxindicator, normalizeL, reset):
        ...
        assert len(self.labels) == self.length, 'The length and the labels is not right {} vs {}'.format(self.length,
                                                                                                         len(
                                                                                                             self.labels))
        assert len(self.NormDistances) == self.length, 'The length and the NormDistances is not right {} vs {}'.format(
    ...
    def _process_(self, image, target, index):
        ...
        if isinstance(theta, list) or isinstance(theta, tuple):
            ...
            affineImage, heatmaps, mask, norm_trans_points, THETA, transpose_theta = \
                torch.stack(affineImage), torch.stack(heatmaps), torch.stack(mask), torch.stack(
                    norm_trans_points), torch.stack(THETA), torch.stack(transpose_theta)
        else:
            affineImage, heatmaps, mask, norm_trans_points, THETA, transpose_theta = self.__process_affine(image,
                                                                                                           target,
                                                                                                           theta,
                                                                                                           nopoints,
                                                                                                           'S[{:}]@{:}'.format(
