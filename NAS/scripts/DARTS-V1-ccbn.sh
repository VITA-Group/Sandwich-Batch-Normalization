#!/bin/bash
# bash scripts/DARTS-V1-ccbn.sh cifar100 0 777
# bash scripts/DARTS-V1-ccbn.sh ImageNet16-120 0 777
echo script name: $0
echo $# arguments
if [ "$#" -lt 3 ] ;then
...
save_dir=./output/search-cell-${space}/DARTS-V1/${dataset}/${seed}/DARTS-ccbn-BN${BN}-es_epoch${early_stop_epoch}
...
OMP_NUM_THREADS=4 python DARTS-V1.py \
	...
	--track_running_stats ${BN} --affine 0 --model DARTS-V1-ccbn \
