...
def get_optim_scheduler(parameters, config, args_weight_lr=None):
    ...
    if config.optim == "SGD":
        ...
        optim = torch.optim.RMSprop(
            parameters, weight_lr, momentum=config.momentum, weight_decay=config.decay
        )
    else:
    ...
    if config.scheduler == "cos":
        ...
        scheduler = CosineAnnealingLR(
            optim, config.warmup, epochs, T_max, config.eta_min
        )
    elif config.scheduler == "multistep":
        scheduler = MultiStepLR(
            optim, config.warmup, epochs, config.milestones, config.gammas
        )
