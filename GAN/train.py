...
def get_network_func(args):
    ...
    if args.dataset == "cifar10":
        networks = {
            ...
            "autogan": (models.AutoganGen32(args), models.SnganDis32(args)),    # using sngan's dis_net
        }
    ...
    assert (
            args.model in networks.keys()
    ), "Networks function '{}' not supported".format(args.model)
...
def main(args):
    ...
    for epoch in tqdm(range(int(start_epoch), int(args.max_epoch)), desc='total progress'):
        lr_schedulers = None
        train_conditional(args, gen_net, dis_net, gen_optimizer, dis_optimizer, gen_avg_param, train_loader, epoch,
                          writer_dict,
                          lr_schedulers)
        ...
        if epoch % args.val_freq == 0 or epoch == int(args.max_epoch) - 1:
            ...
            if fid_score < best_fid:
                best_fid = fid_score
                is_best = True
                logger.info(f'Best performance w.r.t FID: FID score: {fid_score} || @ epoch {epoch}.')
            else:
                is_best = False
        else:
            is_best = False
        ...
        if args.snapshot and epoch % args.snapshot == 0 or epoch == int(args.max_epoch) - 1:
            save_checkpoint(states, False, final_output_dir, f'checkpoint_epoch{epoch + 1}.pth')
